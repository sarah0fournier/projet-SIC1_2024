<template>
  <a-scene
    ...
    <!-- Ajoutez un torus knot -->
  <a-ring color="teal" radius-inner="3" radius-outer="12" position="0 2 -8"> </a-ring>
  ...
  function addCube() {   
    // Ajouter un cube si pas gagner et si jeu pas en pause
     if (isWin.value === false & isPaused.value === false) {
      ...
      const x = getRandomNumberInRange(-9, 9);
      const y = getRandomNumberInRange(-5, 3);
      const z = getRandomNumberInRange(-9, 9);

      newCube.setAttribute('position', `${x} ${y} ${z}`); // Position aléatoire du nouveau cube
      newCube.setAttribute('color', 'pink'); // Couleur du nouveau cube
      ...
      newCube.setAttribute('animation__move', 'property: position; to: 0 0 0; dur: 5000; easing: linear;');
      // newCube.setAttribute('animation__stay', 'property: position; to: -0.01 1.5 1.2; dur: 1; delay: 5000;');
      newCube.setAttribute('animation__disappear', 'property: scale; to: 0 0 0; dur: 10; delay: 5000; easing: linear;');
